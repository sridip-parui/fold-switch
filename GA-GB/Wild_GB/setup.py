...
rest_activation = 0.95 # fraction of restraints to be activated (Please note force constant also depends on frac_activation)
k_model1 = 720.0 # force constant for model1 (here alpha, kb*gamma gamma=Eb/Ea)
k_model2 = 250.0 # force constant for model2 (here beta)
...
def get_CA_restraints(filename, s, ramp, force_constant, scaler, frac_activation):
    r1 = 0.0
    r2 = 0.0
    r3 = 0.8
    r4 = 1.0
...
# CA restraints
    distance_scaler = s.restraints.create_scaler('nonlinear', alpha_min=0.2, alpha_max=0.8, factor=4.0)
    dis = get_CA_restraints(contact[0], s, ramp=LinearRamp(0, 100, 0, 1), force_constant=k_model1,
                        scaler=distance_scaler, frac_activation=rest_activation)
    dis.extend(get_CA_restraints(contact[1], s, ramp=LinearRamp(0, 100, 0, 1), force_constant=k_model2,
                             scaler=distance_scaler, frac_activation=rest_activation))
    print ('dis_group', len(dis))
    s.restraints.add_selectively_active_collection(dis,1)
